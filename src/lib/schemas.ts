...
export const trainerApplicationSchema = z.object({
  username: usernameSchema,
  display_name: z.string().trim().min(1).max(60),
  full_legal_name: z.string().trim().min(1).max(120),
  public_trainer_name: z.string().trim().min(1).max(60),
  country: z.string().trim().min(2).max(80),
  native_language: z.string().trim().min(2).max(40),
  additional_languages: z.array(z.string().trim().max(40)).max(10).default([]),
  specialties: z.array(z.string().trim().min(1).max(40)).min(1).max(10),
  years_experience: z.coerce.number().int().min(0).max(70),
  biography: z.string().trim().min(20).max(2000),
  certification_details: z.string().trim().min(1).max(2000),
  ...
  requested_price: z.coerce.number().min(9.99).max(49.99),
