...
async function runCompleteSeeder() {
  console.log("🚀 Running Complete Seeder for project tdggisdwevfxpitlbeyc...");
  ...
  // 2. Create Trainer Accounts, Profiles, and Trainer Profiles with monetization_enabled = true
  const trainersData = [
  ...
  for (const t of trainersData) {
    ...
    console.log(`Seeding trainer profile & posts for: ${t.displayName} (${t.username})...`);
    ...
    // Trainer Profiles table with monetization_enabled = true & is_verified = true
    const tpRes = await supabase.from("trainer_profiles").upsert({
      user_id: uid,
      value_proposition: t.bio,
    ...
    if (tpRes.error) console.error(`Error trainer_profiles for ${t.username}:`, tpRes.error);

    // Seed 4 sample feed posts for each trainer
    for (let i = 0; i < 4; i++) {
      const media = sampleImages[(trainersData.indexOf(t) + i) % sampleImages.length];
      const pRes = await supabase.from("posts").insert({
        trainer_id: uid,
        kind: i % 2 === 0 ? "feed" : "short",
        caption: `${t.displayName} workout & training breakdown #${i + 1}. Focus on form, intensity, and progressive overload.`,
        media_url: media,
        thumbnail_url: media,
        respect_count: Math.floor(Math.random() * 45) + 12,
        comment_count: Math.floor(Math.random() * 12) + 3,
        is_published: true,
        is_hidden: false,
        is_demo: true,
      });
      if (pRes.error) console.error(`Error post insert for ${t.username}:`, pRes.error);
    }
  }
  ...
  console.log("🎉 Complete Seeding finished! All trainer profiles, posts, community, and demo data are active.");
}

runCompleteSeeder().catch(console.error);
